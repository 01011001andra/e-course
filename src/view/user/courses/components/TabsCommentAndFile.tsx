...
const TabsCommentAndFile = () => {
  const [tabMode, setTabMode] = React.useState<"comments" | "files">("files");
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-600 group-hover:text-white font-light text-sm lg:text-base">
            4
          </span>
        </div>
      </div>
      ...
      {tabMode === "files" && (
        <div className="px-8 py-6 w-full border rounded-3xl">test</div>
      )}
    </div>
  );
};
